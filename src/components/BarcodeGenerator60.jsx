...
const generatePDF = async (barcodes) => {
  ...
  page.drawText(footerText, {
    x: textWidth / 2 - (footerText.length * footerFontSize) / 4, // Center horizontally
    y: footerMargin,
    size: footerFontSize,
  ...
  saveAs(
    ...
    "60-barcodes-with-centered-footer.pdf"
  );
};

const BarcodeGenerator60 = () => {
  // Generate sample barcodes. You can replace these with your own barcode data.
  const barcodes = Array(60).fill("SampleBarcodeData");

  return (
    <button onClick={() => generatePDF(barcodes)}>
      Generate Centered 60 Barcode PDF with Centered Footer
    </button>
  );
};

export default BarcodeGenerator60;
